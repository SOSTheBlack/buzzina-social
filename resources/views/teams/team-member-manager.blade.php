                            ...
                            @foreach ($this->roles as $index => $role)
                                <a href="#"
                                   ...
                                   wire:click.prevent="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                                    <div>
                    <span
                        class="{{ $addTeamMemberForm['role'] == $role->key ? 'fw-medium' : '' }}">
                      {{ $role->name }}
                    </span>
                                        @if ($addTeamMemberForm['role'] == $role->key)
                                            <svg
                                                class="ms-25 text-success fw-light"
                                                width="20"
                                                fill="none"
                ...
                <x-slot
                    ...
                    @foreach ($team->teamInvitations as $invitation)
                        <div
                            ...
                            <div
                                class="___class_+?13___">{{ $invitation->email }}</div>
                    ...
                    @foreach ($team->users->sortBy('name') as $user)
                        <div
                            ...
                            <div
                                ...
                                <span
                                    class="fw-medium">{{ $user->name }}</span>
                            </div>
                            ...
                            <div
                                ...
                                @elseif (Laravel\Jetstream\Jetstream::hasRoles())
                                    <button
                                        class="btn btn-link text-secondary disabled text-decoration-none ms-2">
                                        {{ Laravel\Jetstream\Jetstream::findRole($user->membership->role)->name }}
                                    </button>
                                @endif
                ...
                @foreach ($this->roles as $index => $role)
                    <a href="#"
                       ...
                       wire:click.prevent="$set('currentRole', '{{ $role->key }}')">
                        <div>
              <span
                  class="{{ $currentRole == $role->key ? 'fw-medium' : '' }}">
                {{ $role->name }}
              </span>
